...
<iframe width="560" height="315" src="https://www.youtube.com/embed/X8BPMFGN2IQ" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
Widget
:   A small gadget or mechanical device, especially one whose name is
    unknown or unspecified. (This is not really a vocabulary word, just a fun fact)

Your job is to put together packing lists of widgets when customers
order them. Our goal is to automate this process so we don't have to re-do our packing list every time someone orders something. Let's follow along:
...
1.  Open the **[Google Sheets](https://sheets.google.com/)** app and from this backstage view, select
    **Blank**.
...
This tutorial (and most others) we will follow the common convention in Excel that the first
...
1.  At the bottom of the window, you have only one sheet. Click the plus
    ...
    ![Create a second
    worksheet](images/tutorial1/2.png)
...
4.  Select the **Documentation** sheet before moving on.
    ...
    ![Rename your
    worksheets](images/tutorial1/3.png)
...
1. Ensure that you are on the **Documentation** sheet!
...
1. Switch to the **Packing Slip** sheet.
1.  Select cell **A1** if it is not already selected. Type **Packing
    Slip**. Press **Enter** three times. Cell **A3** should now be selected.
2.  Type **Ship To** and press **Enter**.
3.  For each of the cells from A4:A8, enter the following:
    -   Name
    -   Address
    -   City
    -   State
...
5.  Your final product will like this
    ...
    ![Address section](images/tutorial1/5.png)
...
1.  Select cell **B8**.
    ...
    ![Select cell B8](images/tutorial1/6.png)

2.  Click inside the formula bar and place the insertion point at the
    beginning of the ZIP code before the first **4**.

    ![Place insertion point](images/tutorial1/7.png)
...
3.  Type an apostrophe, the **'** symbol. This is Excel's way of knowing
    ...
    **Enter**, you'll notice that the text left-aligns within the cell
    like the rest of the address.

    ![Text is now left-aligned](images/tutorial1/8.png)
...
1.  Ensure that you are on the **Documentation** sheet. First, we need
    ...
    accommodates all of the data in column **B**. 

    ![Widen column B](images/tutorial1/9.png)

2.  Notice now that the date is right-aligned. This looks funny. But
    instead of converting the date to text, we will simply left-align
    it. Select cell **B4**. Click the **Horizontal Align** tool, and select **Left**.

    ![Left-Align Date](images/tutorial1/10.png)
...
3.  One of the fun things that we can do with numbers is change the way
    ...
    With cell **B4** still selected, open the **Format** menu. Select **Number&gt;More Formats&gt;More date and time formats**

    ![Select formatting for date and time](images/tutorial1/11.png)

1. In the dialog that opens, scroll down to this long date format, select it, and click **Apply**.

    ![Long date format](images/tutorial1/12.png)

    Notice how the date is displayed.
...
widgets that he has ordered.
  ...
  | 8169-64109 | New       | Satin  | hydrocoptic marzel vanes    | 8.75  | 1        |
  | 5468-32757 | New       | Wood   | panametric fans             | 123.4 | 1        |

1.  Click on cell **A10**. In cells **A10:F10** enter the headings for
    our table as follows (this time, press the **tab** key between each
    ...
    ![Enter column headings](images/tutorial1/13.png)
...
2.  Under the headings, enter the data as shown in the table above in
    ...
    ![Enter data](images/tutorial1/14.png)
...
3.  Now, resize the columns so that all of the data fits well. A quick
    ...
    Excel will automatically resize the column to the width of the
    longest content in that column.

    ![Resize columns](images/tutorial1/15.png)

4.  Finally, let's set some number formats. Select cells **E12:E15**. Now, in the drop-down bar, select **Format&gt;Number&gt;Currency**.

    ![Change the number format](images/tutorial1/16.png)

And there we have it. 

![Final product](images/tutorial1/17.png)

Compare yours to the above and if it looks reasonably similar, move on to the next part of the tutorial.
